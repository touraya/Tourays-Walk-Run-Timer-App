...
function renderHome(){
  ...
  const recent48=h.filter(x=>Date.now()-new Date(x.date).getTime()<48*3600000);
  const recentMinutes=Math.round(recent48.reduce((a,x)=>a+(x.duration||0),0)/60);
  const readiness=Math.max(35,Math.min(96,82-Math.min(35,recentMinutes*.38)+Math.min(12,streakCount(h)*2)));
  ...
  if(readiness>=78){
    ...
    H.readinessText.textContent='Your recent activity supports a balanced session.';
    ...
    H.readinessText.textContent='Keep today controlled and focus on consistency.';
    ...
    H.planText.textContent='Choose walking, mobility or a light core session.';
  }

  H.runSubtitle.textContent=week.some(x=>x.type==='Walk & Run')?'Continue building endurance':'Intervals with live GPS';
  H.indoorSubtitle.textContent=week.some(x=>x.type!=='Walk & Run')?'Keep your strength streak':'8 guided exercises';
  ...
  if(h[0]){
    ...
    H.recentText.textContent='Your latest completed workout will appear here.';
...
document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>show(b.dataset.screen));H.repeatWorkout.onclick=()=>{if(H.repeatWorkout.disabled)return;show(H.repeatWorkout.dataset.type==='Walk & Run'?'run':'indoor')};
document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>show(b.dataset.go));document.querySelectorAll('.preset').forEach(b=>b.onclick=()=>{setTime(+b.dataset.w,E.wm,E.ws);setTime(+b.dataset.r,E.rm,E.rs);document.querySelectorAll('.preset').forEach(x=>x.classList.toggle('selected',x===b))});
